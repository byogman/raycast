...
export function selectUrl(item: any) {
  ...
  } else if (item.metadata.external_system_identifiers && item.metadata.external_system_identifiers.some((obj: any) => obj.schema === "KEKSCAN")) {
    const kekValue: string = item.metadata.external_system_identifiers.find((obj: any) => obj.schema === "KEKSCAN").value;
    const parts: string[] = kekValue.split('-');
    const urlComponent: string = (parts[0].length === 4 ? parts[0] : "19" + parts[0]) + "/" + parts[0].slice(-2) + parts[1] + "/" + parts[0].slice(-2) + parts[1] + parts[2] + ".pdf";
